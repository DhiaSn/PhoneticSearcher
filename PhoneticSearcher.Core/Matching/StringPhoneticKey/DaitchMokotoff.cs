...
namespace PhoneticSearcher.Core.Matching.StringPhoneticKey
{
    ...
    public class DaitchMokotoff : StringPhoneticKeyBuilder
    {
        public override string BuildKey(string str1)
        {
            ...
            return BuildDaitchMokotoff(str1);
        }
        ...
        private string BuildDaitchMokotoff(string str1)
        {
            ...
            while (daimokRules[n].Length > 0 && str1.Length > 0)
            {
                if (str1.RangeMatches(pos, daimokRules[n], 0, daimokRules[n].Length))
                {
                    /* check the position of the sound */
                    if (pos == 0)
                    {
                        /* the beginning */
                        sound = daimokRules[n + 1];
                        pos = pos + daimokRules[n].Length;
                    }
                    else
                    {
                        pos = pos + daimokRules[n].Length;
                        ...
                        {
                            /* vor einem Vokal */
                            sound = daimokRules[n + 2];
                        }
                        else
                        {
                            sound = daimokRules[n + 3];
                        }
                    }
