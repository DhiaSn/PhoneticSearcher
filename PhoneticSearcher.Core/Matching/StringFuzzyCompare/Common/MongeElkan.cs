...
namespace PhoneticSearcher.Core.Matching.StringFuzzyCompare.Common
{
    ...
    public class MongeElkan : StringFuzzyComparer
    {
        ...
        {
            get { return useSpecialAbbreviationCheck; }
            set { useSpecialAbbreviationCheck = value; }
        }
        ...
        public override float Compare(string str1, string str2)
        {
            ...
            string[] tokens1 = tokenizer.Tokenize(str1);

            // "Henry Peter Waternose" -> "Henry", "Peter", "Waternose"
            string[] tokens2 = tokenizer.Tokenize(str2);

            float distance = BuildMongeElkan(tokens1, tokens2);
        ...
        private float BuildMongeElkan(string[] tokens1, string[] tokens2)
        {
            ...
            for (int i = 0; i < tokens1.Length; i++)
            {
                ...
                {
                    similarity = CompareTokens(tokens1[i], tokens2[j]);
            ...
            float mongeElkan = maxSimilarities.Sum() / tokens1.Length;
        ...
        private float CompareTokens(string token1, string token2)
        {
            ...
            // standard compare
            float similarity = fuzzyComparer.Compare(token1, token2);
            return similarity;
        }
    }
}