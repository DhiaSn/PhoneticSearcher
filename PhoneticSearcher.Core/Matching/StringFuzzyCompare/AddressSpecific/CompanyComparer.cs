...
namespace PhoneticSearcher.Core.Matching.StringFuzzyCompare.AddressSpecific
{
    ...
    public class CompanyComparer : StringFuzzyComparer
    {
        ...
        {
            string name1 = Normalize(str1);
            string name2 = Normalize(str2);

            // check if company is shortened like "International Business Machines" -> "IBM"
            string firstChars1 = GetFirstCharsFromWords(name1).Join("");
            string firstChars2 = GetFirstCharsFromWords(name2).Join("");
